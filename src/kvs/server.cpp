...
void run(unsigned thread_id, Address public_ip, Address private_ip,
  ...
  management_node_response_puller.bind(wt.management_node_response_bind_address());
  ...
  while (true) {
    ...
    if (duration >= kServerReportThreshold) {
      ...
      // local mode.
      if (management_ip != "NULL") {
        kZmqUtil->send_string(wt.management_node_response_connect_address(), &pushers[get_func_nodes_req_address(management_ip)]);
      }
